...
const NewFunctionScreen = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      swalAlert(
        `Función "${formValues.nombreFuncion}" creada correctamente`,
        "success"
      );
    } catch (err) {
      console.log(err);
      swalAlert(
        "Errores sintácticos de la función o no estás declarando una!",
        "warning"
      );
    }
  };
  ...
  const handleModalSubmit = async (e) => {
    ...
    const { code, content } = await postNewCategory({
